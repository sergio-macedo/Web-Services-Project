...
public class ResourceExceptionHandler {
    ...
    public ResponseEntity<StandardError> resourceNotFound(ResourceNotFoundException e, HttpServletRequest request) {
        ...
        HttpStatus status = HttpStatus.NOT_FOUND;// will try this one at the response from others.
    ...
    public ResponseEntity<StandardError> database (DatabaseException e, HttpServletRequest request) {
        ...
        HttpStatus status = HttpStatus.BAD_REQUEST;// will try this one at the response from others.
        StandardError error1 = new StandardError(Instant.now(), status.value(), error, e.getMessage(), request.getRequestURI());
        return ResponseEntity.status(status).body(error1);
    }
}